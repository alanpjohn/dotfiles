...
This will display all configured application names, such as tmux, zed, ghostty, nvim, opencode, fish, and matugen.
...
This configuration syncs only the essential fish shell files (`config.fish` and `fish_plugins`), excluding plugin-managed directories and machine-specific variables.
...
- `fish` - Fish shell configuration (config.fish and fish_plugins only)
- `matugen` - Matugen Material You theme generator configuration and templates
...
This dotfiles setup integrates with [Matugen](https://github.com/InioX/matugen), a Material You color generation tool. Matugen generates theme files that are output into other applications' config directories. To avoid overlap, the following boundary strategy is enforced:

### Boundary Rule

**Each application's sync entry only includes configuration that is NOT generated by Matugen.** Matugen-generated files are excluded from their source app's sync and are instead tracked via the `matugen` entry (which includes the templates that produce them).

### Matugen-Generated Files

| Application | Generated File | Sync Status |
|-------------|---------------|-------------|
| starship | `starship.toml` | **Not synced** — entirely matugen-generated. Template: `matugen/templates/starship-colors.toml` |
| tmux | `generated.conf` | **Excluded** from tmux sync. Template: `matugen/templates/tmux-colors.conf` |
| nvim | `matugen.lua` | **Excluded** from nvim sync. Template: `matugen/templates/neovim/template.lua` |
| ghostty | `themes/matugen` | No conflict — output is in `themes/` subdir, not the main config |
| zed | `themes/matugen.json` | No conflict — `themes/` already in zed's exclude list |
| opencode | `themes/matugen.json` | No conflict — output is in `themes/` subdir |

### How It Works

1. **Matugen templates** are tracked under `matugen/templates/` in the repo
2. When you run `matugen generate`, it reads the templates and outputs generated files to their target locations
3. The sync script **excludes** these generated files from their source app's sync to prevent conflicts
4. Starship is not synced at all since its entire configuration is matugen-generated

### Adding New Matugen Templates

When adding a new Matugen template that generates a file in another app's config directory:

1. Add the template to `matugen/templates/`
2. Add the matugen config entry in `~/.config/matugen/config.toml`
3. Add the generated filename to the target app's `exclude` list in `sync-config.json`
4. If the generated file is the app's entire config (like starship), remove the app from sync entirely
